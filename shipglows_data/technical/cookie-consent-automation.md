...
Supported rules: OneTrust, Cookiebot, Didomi, Quantcast, Axeptio, SourcePoint, Google/YouTube, Meta/Instagram, TikTok (including Shadow DOM banner), and generic standardized consent attributes (`[data-consent="accept"]`, `[aria-label="Accept all"]`, etc.). Only visible enabled interactive controls are eligible. Clicks use robust PointerEvents (`pointerdown`, `pointerup`, `click`) compatible with modern reactive frameworks (React, Vue). There is no arbitrary text scan, cookie inspection/write, fingerprint spoofing, anti-bot bypass, or remote rule download. Existing storage and content-script permissions suffice; no new permissions or bridge messages.
